 ...
 * Fixed version with proper Puppeteer selectors
 * 
 * Run with: node src/test/e2e/complete-workflow-test-fixed.js
 */
...
// Helper function to click element by text content
async function clickByText(text, timeout = 5000) {
  try {
    await page.evaluate((searchText) => {
      // Look for clickable elements including cards, buttons, links
      const elements = Array.from(document.querySelectorAll('button, a, [role="button"], [class*="clickable"], [class*="cursor-pointer"]'));
      const element = elements.find(el => el.textContent.includes(searchText));
      if (element) {
        element.click();
        return true;
      }
      // If not found in clickable elements, search all elements
      const allElements = Array.from(document.querySelectorAll('*'));
      const fallbackElement = allElements.find(el => {
        return el.textContent.trim().includes(searchText) && 
               (el.onclick || el.classList.contains('cursor-pointer') || el.classList.contains('clickable'));
      });
      if (fallbackElement) {
...
async function step2_navigateToRaceMaintenance() {
  ...
  try {
    // Find and click the Race Maintenance card
    await page.evaluate(() => {
      const headings = Array.from(document.querySelectorAll('h3'));
      const raceMaintenanceHeading = headings.find(h => h.textContent.includes('Race Maintenance'));
      if (raceMaintenanceHeading) {
        // Find the parent card and click it
        let parent = raceMaintenanceHeading.parentElement;
      ...
      throw new Error('Race Maintenance card not found');
    });
    
    await waitForText('Race Setup');
...
async function step4_configureRunners() {
  ...
  try {
    ...
    await waitForText('Runner Configuration');
    await page.waitForTimeout(1000);
...
async function step5_createRace() {
  ...
  try {
    ...
    const runnerCountText = await page.evaluate(() => {
      const cards = Array.from(document.querySelectorAll('.card, [class*="card"]'));
      for (const card of cards) {
        if (card.textContent.includes('Total Runners')) {
          const numberEl = card.querySelector('.text-2xl, [class*="text-2xl"]');
          return numberEl ? numberEl.textContent.trim() : null;
        }
      }
      return null;
    });
...
async function step6_openImportExportModal() {
  ...
  try {
    ...
    await clickByText('Import / Export');
    await waitForText('Import / Export Race Configuration');
    await page.waitForTimeout(1000);
...
async function step7_exportConfiguration() {
  ...
  try {
    ...
    await clickByText('Export Configuration');
    await page.waitForTimeout(500);
    
    // Click Export Race Configuration button
    await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      const exportBtn = buttons.find(btn => btn.textContent.includes('Export Race Configuration'));
...
async function step10_clearDatabase() {
  ...
  try {
    ...
    await page.evaluate(() => {
      const elements = Array.from(document.querySelectorAll('h3, h2, h4'));
      const dangerZone = elements.find(el => el.textContent.includes('Danger Zone'));
      if (dangerZone) dangerZone.scrollIntoView();
    });
    await page.waitForTimeout(500);
    
    // Click Clear Database button
    await clickByText('Clear Database');
    await page.waitForTimeout(500);
    ...
    await clickByText('Confirm Clear Database');
...
async function step11_openImportModal() {
  ...
  try {
    ...
    await clickByText('Import Configuration');
    await page.waitForTimeout(500);
...
async function step12_importConfiguration() {
  ...
  try {
    ...
    if (textareas.length > 0) {
      await textareas[0].type(JSON.stringify(exportedConfig));
    }
    await page.waitForTimeout(1000);
    ...
    await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      const importBtns = buttons.filter(btn => btn.textContent.includes('Import Configuration'));
      if (importBtns.length > 1) {
        importBtns[1].click(); // Second button is the action button
      }
    });
...
async function step13_verifyImportedRace() {
  ...
  try {
    ...
    const hasOverview = await page.evaluate(() => document.body.textContent.includes('Overview'));
    if (!hasOverview) {
      // Navigate to race overview
      await clickByText('Race Maintenance');
      await page.waitForTimeout(1000);
    }
    ...
    // Verify checkpoints
    const checkpointCount = await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      return buttons.filter(btn => btn.textContent.includes('Checkpoint')).length;
    });
    ...
    const verificationResults = {
      ...
      checkpointsPresent: checkpointCount > 0
    };
...
async function step14_testImportedRaceFunctionality() {
  ...
  try {
    ...
    await clickByText('Checkpoint 1');
    await waitForText('Checkpoint');
    await page.waitForTimeout(1000);
    ...
    await clickByText('Go to Basestation');
    await waitForText('Base Station');
    await page.waitForTimeout(1000);
