...
const CheckpointCallInPage = ({ checkpointNumber }) => {
  const {
    getCheckpointRunners,
    markCheckpointRunner,
    bulkMarkCheckpointRunners,
  ...
  React.useEffect(() => {
    if (checkpointNumber) {
      loadCheckpointRunners(checkpointNumber);
    }
  }, [checkpointNumber, loadCheckpointRunners]);

  const checkpointRunners = getCheckpointRunners(checkpointNumber);
  
  // Group runners by status
  const groupedRunners = useMemo(() => {
    const notStarted = checkpointRunners.filter(r => r.status === 'not-started');
    const passed = checkpointRunners.filter(r => r.status === 'passed');
    const dnf = checkpointRunners.filter(r => r.status === 'dnf');
    const nonStarter = checkpointRunners.filter(r => r.status === 'non-starter');
  ...
  const timeSegments = useMemo(() => {
    const passedRunners = checkpointRunners.filter(r => r.status === 'passed' && r.markOffTime);
  ...
  const handleBulkMark = async () => {
    ...
    try {
      await bulkMarkCheckpointRunners(
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
          Checkpoint {checkpointNumber} Call-In
        </h3>
        ...
        <div className="card p-4">
          ...
          <div className="text-sm text-gray-600 dark:text-gray-300">
            Selected: {formatRunnerList(
              checkpointRunners.filter(r => selectedRunners.has(r.number))
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <span className="ml-2 font-medium">{checkpointRunners.length}</span>
          </div>
          <div>
...
export default CheckpointCallInPage;
